    ...
    if filter_col == '':
        if group_similar_String=='':
            similar_Query = f"""
                ...
                {Similarity_string}
                from search_source) as new_table
...
def CreateGroupSimilarString(att_group_info, group_similar, fuzzyAtt, customer_columns, address_columns, phone_columns, email_columns, row, i):
    ...
    for att in group_similar:
        if row[att]!="":
            if att_group_info[str(att)]['label']=='phone':
                for master_att in master_phone_columns.split():
                    group_similar_String += """SIMILARITY("{}"::text, '{}') AS score_{}_{}_{},""".format(master_att, row[att],att_group_info[att]['type'],att,master_att)
            else:
                for master_att in att_group_info[att]['data']:
                    ...
                    if att in email_columns:
                        if att_group_info[att]['process']=='email':
                            group_similar_String += """SIMILARITY(REGEXP_REPLACE("{}"::text, '@.*', ''),REGEXP_REPLACE('{}', '@.*', '')) AS score_{}_{}_{},""".format(master_att, row[att],att)
                        else:
                            group_similar_String += """SIMILARITY("{}"::text, '{}') AS score_{}_{}_{},""".format(master_att, row[att],fuzzyAtt[i])
                    else:
                        group_similar_String += """SIMILARITY("{}"::text, '{}') AS score_{}_{}_{},""".format(master_att, row[att], att_group_info[att]['type'],att,master_att)
