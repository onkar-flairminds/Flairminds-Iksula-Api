...
def createSimilarQuery(filter_col, filter_val, Similarity_string, Addition_String, group_similar_String, Add_count, mode):
    if mode=='customer':
        table_string = f"""{customer_table} as customer 
    ...
    for att in group_similar:
        if row[att]!="":
            if mode=='customer':
                ...
                for master_att in att_group_info[att]['data']:
                    group_score_list.append('score_{}_{}_{}'.format(att_group_info[att]['type'].lower(),att.lower(),master_att.lower()))
                    group_similar_String += """SIMILARITY(product."{}"::text, '{}') AS score_{}_{}_{},""".format(master_att, removeStringBlockers(row[att]), att_group_info[att]['type'],att,master_att)
    ...
    for i in range(len(fuzzyAtt)):
        if fuzzyValue[i]!="":
            ...
            if mode=='customer':
                ...
                Similarity_string += """SIMILARITY(product."{}"::text, '{}') AS score_{},""".format(fuzzyAtt[i], removeStringBlockers(fuzzyValue[i]),fuzzyAtt[i])
    ...
    for att in group_exact:
        if mode=='customer':
            ...
            for master_att in att_group_info[att]['data']:
                    if type(row[att])==str:
                        if row[master_att]!="":
                            group_exact_String += """product."{}"::text='{}' or """.format(master_att, removeStringBlockers(row[att]),att)
...
def createExactstring(exactAtt, exactValue, customer_columns, address_columns, phone_columns, email_columns, mode):
    ...
    for i in range(len(exactAtt)):
        if mode=='customer':
            ...
            if type(exactValue[i])==str:
                if exactValue[i]!="":
                    exact_String += """product."{}"='{}' or """.format(exactAtt[i], removeStringBlockers(exactValue[i]),exactAtt[i])
            else:
                exact_String += """product."{}"={} or """.format(exactAtt[i], exactValue[i],exactAtt[i])
